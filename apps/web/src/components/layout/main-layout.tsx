		...
		<div className="flex h-[calc(100vh)] w-full flex-col overflow-x-hidden bg-gradient-to-br from-background to-muted/20 sm:h-[calc(100vh-18px)]">
			...
			{headers.map((header, idx) => (
				<div
					...
					className="w-full border-border/50 border-b bg-background/80 backdrop-blur-sm"
				...
				{disableScrollArea ? (
					...
					<ScrollArea.Root className="h-full w-full">
						...
						<div className="hidden sm:block">
							...
							>
								<ScrollArea.Thumb className="w-full rounded-full bg-primary transition-colors hover:bg-primary/80" />
							</ScrollArea.Scrollbar>
						</div>
					</ScrollArea.Root>
				)}
