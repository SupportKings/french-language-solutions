...
async function getTouchpoint(id: string) {
	...
	const touchpoint = {
		...
		type: "outbound",
		message:
			"Hi! This is a follow-up message regarding your French language course enrollment. How are you finding the materials so far?",
		source: "automated",
				...
				badges={[
					{
						...
						variant:
							channelColors[touchpoint.channel as keyof typeof channelColors],
					},
					{
						label:
					...
					},
					{
						...
						variant:
							sourceColors[touchpoint.source as keyof typeof sourceColors],
					},
				]}
				...
				<div className="grid gap-4 lg:grid-cols-3">
									...
									<InfoField
										...
										value={
											<Badge
												variant={
													sourceColors[
														touchpoint.source as keyof typeof sourceColors
													]
												}
											>
								...
								<div className="border-t pt-4">
									<InfoSection title="Student" icon={User}>
										...
										{touchpoint.student.email && (
											<InfoField
												label="Email"
												value={touchpoint.student.email}
												icon={Mail}
											/>
										)}
										{touchpoint.student.mobile_phone_number && (
											<InfoField
												label="Phone"
												value={touchpoint.student.mobile_phone_number}
												icon={Phone}
											/>
										)}
									</InfoSection>
											...
											{externalData && (
												<>
													{externalData.provider && (
														<InfoField
															label="Provider"
															value={externalData.provider}
														/>
													)}
													{externalData.message_id && (
														<InfoField
															label="Message ID"
															value={externalData.message_id}
														/>
													)}
												</>
											)}
					...
					<div className="space-y-4">
						...
						<SystemInfoCard
							...
							additionalFields={[{ label: "Source", value: touchpoint.source }]}
						/>
					</div>
				</div>
			</DetailViewContent>
