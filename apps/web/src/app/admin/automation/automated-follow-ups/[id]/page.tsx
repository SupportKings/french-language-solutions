...
async function getFollowUp(id: string) {
	...
	const followUp = {
		...
		last_message_sent_at: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), // 1 day ago
		completed_at: null,
		...
		touchpoints: [
			{
				id: "touch-1",
				message: "Welcome to French Language Solutions! We're excited to have you.",
				channel: "sms",
				type: "outbound",
...
export default async function FollowUpDetailPage({
	...
	// Calculate progress
	const progress = followUp.sequence.total_messages > 0 
		? (followUp.sequence.messages_sent / followUp.sequence.total_messages) * 100 
		: 0;

	// Calculate time since last message
	const timeSinceLastMessage = followUp.last_message_sent_at
		? formatDistanceToNow(new Date(followUp.last_message_sent_at), { addSuffix: true })
		: "No messages sent";
	...
	return (
		<DetailViewLayout>
			...
			<DetailViewHeader
				...
				actions={[
					{
						icon: followUp.status === 'activated' || followUp.status === 'in_progress' ? Pause : Play,
						label: followUp.status === 'activated' || followUp.status === 'in_progress' ? "Pause" : "Resume",
						...
						<div className="space-y-4">
							...
							<div>
								<div className="flex items-center justify-between text-sm mb-2">
									...
									<span className="font-medium">{followUp.sequence.messages_sent} / {followUp.sequence.total_messages}</span>
								</div>
							...
							<div className="grid gap-4 sm:grid-cols-3">
								...
								<div className="rounded-lg border bg-muted/10 p-3">
									...
									<p className="text-sm font-medium">{timeSinceLastMessage}</p>
								</div>
								<div className="rounded-lg border bg-muted/10 p-3">
									<div className="flex items-center gap-2 mb-1">
									...
									<p className="text-sm font-medium">
										{followUp.status === 'activated' || followUp.status === 'in_progress' 
											? "Scheduled" 
											: followUp.status === 'completed' ? "Completed" : "Paused"}
									</p>
								</div>
							</div>
						</div>
					...
					>
						<div className="space-y-2 max-h-64 overflow-y-auto">
							{followUp.touchpoints.map((touchpoint) => {
								...
								return (
									<div key={touchpoint.id} className="rounded-lg border bg-muted/10 hover:bg-muted/20 transition-colors p-3">
					...
					>
						<div className="space-y-3">
							<div className="rounded-lg border bg-muted/10 p-3">
								...
								<p className="text-sm font-medium">{followUp.sequence.display_name}</p>
							</div>
							<div className="rounded-lg border bg-muted/10 p-3">
								<p className="text-xs text-muted-foreground mb-1">Subject</p>
								<p className="text-sm font-medium">{followUp.sequence.subject}</p>
							</div>
							<div className="rounded-lg border bg-muted/10 p-3">
							...
							<CardContent className="space-y-4">
								...
								<InfoSection title="Student" icon={User}>
									<InfoField 
										label="Name" 
										value={
											<Link href={`/admin/students/${followUp.student_id}`} className="text-primary hover:underline">
									...
									{followUp.student.mobile_phone_number && (
										<InfoField label="Phone" value={followUp.student.mobile_phone_number} />
									)}
									...
									<InfoSection title="Timeline" icon={Calendar}>
										...
										{followUp.last_message_sent_at && (
											<InfoField 
												...
												value={format(new Date(followUp.last_message_sent_at), "PPpp")} 
												icon={Send} 
											/>
										)}
