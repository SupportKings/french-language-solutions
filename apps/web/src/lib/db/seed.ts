...
async function seed() {
	...
	const teachersData = Array.from({ length: 10 }, () => ({
		...
		groupClassBonusTerms: faker.helpers.arrayElement([
	...
	const productsData = [
		{
			displayName: "Beginner French A1",
			location: "online" as const,
			format: "group" as const,
		},
		{
			displayName: "Beginner French A2",
			location: "online" as const,
			format: "group" as const,
		},
		{
			displayName: "Intermediate French B1",
			location: "online" as const,
			format: "group" as const,
		},
		{
			displayName: "Intermediate French B2",
			location: "online" as const,
			format: "group" as const,
		},
		{
			displayName: "Advanced French C1",
			location: "online" as const,
			format: "group" as const,
		},
		{
			displayName: "Private Tutoring",
			location: "online" as const,
			format: "private" as const,
		},
		{
			displayName: "Business French",
			location: "hybrid" as const,
			format: "group" as const,
		},
		{
			displayName: "Conversation Club",
	...
	const cohortsData = Array.from({ length: 15 }, (_, i) => ({
		...
		cohortStatus: faker.helpers.arrayElement([
			"enrollment_open",
			"enrollment_closed",
			"class_ended",
		] as const),
	...
	const insertedAutomated = await db
		.insert(automatedFollowUps)
		.values(automatedData)
		.returning();
