...
export const studentSummary: StudentSummary = {
	...
	avgHomeworkCompletionRate: Math.round(
		students.reduce((sum, s) => sum + s.homeworkCompletionPercentage, 0) /
			students.length,
	),
	studentsUnder60Attendance: students.filter((s) => s.attendancePercentage < 60)
		.length,
	studentsNotSubmittingHomework: students.filter(
		(s) => s.homeworkCompletionPercentage < 30,
	).length,
	studentsFallingBehind: students.filter(
...
export const getLevelColor = (level: string): string => {
	...
	if (level.startsWith("C1") || level.startsWith("C2"))
