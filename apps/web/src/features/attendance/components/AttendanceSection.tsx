...
export function AttendanceSection({ cohortId }: AttendanceSectionProps) {
	...
	const updateAttendance = async (
		recordId: string,
		updates: { status?: string; homeworkCompleted?: boolean },
	) => {
		try {
	...
	const getAvailableClassesForAttendance = () => {
		return classes.filter((cls: any) => {
			...
			const studentsWithAttendance = new Set(
				classAttendance.map((r) => r.studentId),
			);
	...
	const createAttendanceRecords = async () => {
		...
		try {
			...
			if (studentsNeedingAttendance.length === 0) {
				toast.info(
					"All students already have attendance records for this class",
				);
				return;
			}
	...
	const getStatusBadge = (status: string) => {
		...
		const config =
			statusConfig[status as keyof typeof statusConfig] || statusConfig.unset;
	...
	if (attendanceRecords.length === 0) {
		return (
			<div className="rounded-lg bg-muted/30 py-8 text-center">
				...
				<p className="mb-4 text-muted-foreground">No attendance records yet</p>
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between gap-4">
				<div className="flex items-center gap-3">
					<Select
						value={groupBy}
						onValueChange={(value: "student" | "class") => setGroupBy(value)}
					>
				...
				<div className="flex items-center gap-4 text-sm">
					...
					<div className="flex items-center gap-2">
						...
						<span>
							{
								attendanceRecords.filter((r) => r.status === "not_attended")
									.length
							}{" "}
							Absent
						</span>
					...
					<div className="flex items-center gap-2">
						...
						<span>
							{attendanceRecords.filter((r) => r.status === "unset").length} Not
							Marked
						</span>
					</div>
					<div className="flex items-center gap-2">
			...
			<div className="space-y-4">
				{sortedGroups.map(([groupName, records]) => {
					...
					const attendanceRate =
						totalCount > 0
							? ((attendedCount / totalCount) * 100).toFixed(0)
							: 0;
									...
									<div className="flex items-center gap-3">
										...
										{groupBy === "class" && records[0]?.class && (
											<span className="text-muted-foreground text-sm">
												{format(
													new Date(records[0].class.start_time),
										...
										<div className="flex items-center justify-between">
											...
											<div className="flex items-center gap-3">
												...
												{record.status === "attended" && (
													<div
														className={`flex cursor-pointer items-center gap-1.5 rounded border px-2 py-1 text-xs transition-all duration-200 hover:opacity-80 ${
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							...
							students. This will create attendance entries with "Not Marked"
							status that you can update later.
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4 py-4">
							...
							>
								...
								<SelectContent>
									{getAvailableClassesForAttendance().map((cls) => {
										...
										const existingForClass = attendanceRecords.filter(
											(r) => r.classId === cls.id,
										);
										const remainingStudents =
											enrolledStudents.length -
											new Set(existingForClass.map((r) => r.studentId)).size;

										return (
											<SelectItem key={cls.id} value={cls.id}>
												<div className="flex flex-col">
													<span>
														{format(classDate, "MMM d, yyyy 'at' h:mm a")}
													</span>
						...
						{selectedClassId && (
							<div className="space-y-2 rounded-lg bg-muted/50 p-3">
								...
								{(() => {
									...
									const studentsWithAttendance = new Set(
										existingForClass.map((r) => r.studentId),
									);
									const studentsNeedingAttendance = enrolledStudents.filter(
										(enrollment) =>
									...
									return (
										<>
											<div className="flex items-center gap-2 text-muted-foreground text-sm">
												...
												<span>
													{studentsNeedingAttendance.length} students (out of{" "}
												...
												<span>
													{(() => {
														...
														return selectedClass
															? format(
																	new Date(selectedClass.start_time),
																	"MMM d, yyyy 'at' h:mm a",
																)
															: "";
													})()}
						...
						{getAvailableClassesForAttendance().length === 0 && (
							<div className="py-4 text-center text-muted-foreground text-sm">
								...
								<p className="mt-1">
									Either all classes already have attendance records or there
