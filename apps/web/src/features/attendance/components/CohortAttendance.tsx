...
export function CohortAttendance({ cohortId }: CohortAttendanceProps) {
	...
	const createAttendanceRecords = async () => {
		...
		try {
			...
			const studentsNeedingAttendance = enrolledStudents
				.filter((student) => !studentsWithAttendance.has(student.student_id || student.id))
			...
			const attendanceData = studentsNeedingAttendance.map((studentId) => ({
