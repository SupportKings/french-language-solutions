...
export function useLanguageLevels() {
	const { data: levels = [], isLoading } = useQuery(languageLevelQueries.list());
	...
	const enumToIdMap = levels.reduce((acc, level) => {
	...
	const getLevelByCode = (code: string) => levels.find(level => level.code === code);
