...
if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
	throw new Error(
		"Missing Airtable configuration. Please set AIRTABLE_API_KEY and AIRTABLE_BASE_ID",
	);
}
...
function trackSkippedRecord(
	...
	importStats.tables[table].skipped =
		(importStats.tables[table].skipped || 0) + 1;
...
function mapMultiSelectToEnumArray(
...
function matchAirtableLevelIdToSupabase(
	...
	console.warn(
		`No matching Supabase level found for Airtable record ID: ${airtableRecordId}`,
	);
	return null;
}
...
async function matchLanguageLevels() {
	...
	for (const level of existingLevels || []) {
		...
		if (level.airtable_record_id) {
			...
			console.log(
				`  Mapped Airtable ID ${level.airtable_record_id} → ${level.display_name} (${level.id})`,
			);
		}
	...
	console.log(
		`✅ Created ${languageLevelMapping.size} language level mappings`,
	);
}
...
async function importTeachers() {
	...
	for (const record of records) {
		...
		const teacher = {
			...
			onboarding_status: mapEnum(
				fields["Team Onboarding Status"],
				"onboarding_status",
			),
			contract_type: mapEnum(fields["Contract Type"], "contract_type"),
			group_class_bonus_terms: mapEnum(
				fields["Group Class Bonus Terms"],
				"group_class_bonus_terms",
			),
			max_students_in_person:
				fields["Maximum Students Per In-Person Class"] || null,
			max_students_online:
				fields["Maximum Students for Online Group Class"] || null,
			available_for_online_classes: mapYesNoToBoolean(
				fields["Available for Teach Online Classes"],
			),
			available_for_in_person_classes: mapYesNoToBoolean(
				fields["Available for In-Person Classes"],
			),
			...
			days_available_in_person:
				fields["Days Available for In-Person Classes"]
					?.map((d: string) => mapEnum(d, "day_of_week"))
					.filter(Boolean) || null,
			days_available_online:
				fields["Days Available for Online Classes"]
					?.map((d: string) => mapEnum(d, "day_of_week"))
					.filter(Boolean) || null,
			available_for_booking:
				fields["Available for Booking?"] === "Available" ? true : false,
			qualified_for_under_16: mapYesNoToBoolean(
...
async function importStudents() {
	...
	for (const record of records) {
		...
		const student: any = {
			...
			communication_channel:
				mapEnum(
					fields["Default Communication Channel"],
					"communication_channel",
				) || "email",
			initial_channel: mapEnum(fields["Initial Channel"], "initial_channel"),
			is_full_beginner:
				fields["Student's Beginning Level (from Enrollment Form)"] ===
	...
	for (const record of records) {
		...
		const product = {
			...
			pandadoc_contract_template_id:
				fields["Contract Template ID (PandaDoc)"] || null,
			signup_link_for_self_checkout:
				fields["Signup Link (for Self-Checkout)"] || null,
			airtable_record_id: record.id,
		};
...
async function importLanguageLevels() {
	...
	importStats.tables.language_levels = {
...
async function importTemplateFollowUpSequences() {
	...
	importStats.tables.template_follow_up_sequences = {
	...
	if (sequences.length > 0) {
		...
		if (error) {
			...
			importStats.errors.push({
				table: "template_follow_up_sequences",
				error: error.message,
			});
			console.error("  ❌ Failed to import sequences:", error.message);
		} else {
			importStats.tables.template_follow_up_sequences.succeeded =
				data?.length || 0;
...
async function importCohorts() {
	...
	for (const record of records) {
		...
		if (fields["Max Students"]) {
			maxStudents =
				typeof fields["Max Students"] === "number"
					? fields["Max Students"]
					: null;
		}
		...
		const cohort = {
			...
			cohort_status:
				mapEnum(fields["Cohort Status"], "cohort_status") || "class_ended",
			max_students: maxStudents,
			room_type: mapEnum(
				fields["Max Students - Restricted by Room (Manual)"],
				"room_type",
			),
...
async function importEnrollments() {
	...
	console.log(
		`  Found ${enrollments.length} enrollments for Pass 2 processing`,
	);
	return enrollments;
}
...
async function importStudentAssessments() {
	...
	importStats.tables.student_assessments = {
	...
	console.log(
		`  Found ${assessments.length} assessments for Pass 2 processing`,
	);
	return assessments;
}
...
async function importAutomatedFollowUps() {
	...
	for (const record of records) {
		...
		const followUp = {
			...
			started_at:
				convertToISO8601(fields["Activated Time"]) || new Date().toISOString(),
			last_message_sent_at: convertToISO8601(fields["Last Follow Up Time"]),
			completed_at:
				fields["Status"] === "100 - Completed"
					? convertToISO8601(fields["Last Follow Up Time"])
					: null,
			airtable_record_id: record.id,
			// Store Airtable references for Pass 2
			_airtable_student_id: fields["Student"]?.[0] || null,
	...
	importStats.tables.automated_follow_ups = {
	...
	console.log(
		`  Found ${followUps.length} automated follow ups for Pass 2 processing`,
	);
	return followUps;
}
...
async function importTouchpoints() {
	...
	for (const record of records) {
		...
		const touchpoint = {
			...
			_airtable_automated_follow_up_id:
				fields["Automated Follow Up"]?.[0] || null,
		};
	...
	importStats.tables.touchpoints = {
		attempted: touchpoints.length,
		succeeded: 0,
		failed: 0,
		skipped: 0,
		skippedReasons: {},
	...
	console.log(
		`  Found ${touchpoints.length} touchpoints for Pass 2 processing`,
	);
	return touchpoints;
}
...
async function importWeeklySessions() {
	...
	for (const record of records) {
		...
		if (!dayOfWeekString) {
			trackSkippedRecord(
				"weekly_sessions",
				record.id,
				"Day of Week (String) field is missing",
				{
		...
		if (!dayOfWeek) {
			trackSkippedRecord(
				"weekly_sessions",
				record.id,
				"Could not map Day of Week value",
				{
		...
		if (typeof startTimeDuration === "number") {
			...
			trackSkippedRecord(
				"weekly_sessions",
				record.id,
				"Start time is not a duration number",
				{
		...
		if (endTime.includes("AM") || endTime.includes("PM")) {
			...
			if (match) {
				...
				trackSkippedRecord(
					"weekly_sessions",
					record.id,
					"Could not parse end time format",
					{
						endTime: endTime,
					},
				);
	...
	importStats.tables.weekly_sessions = {
	...
	console.log(
		`  Found ${sessions.length} weekly sessions for Pass 2 processing`,
	);
	return sessions;
}
...
async function importClasses() {
	...
	for (const record of records) {
		...
		const classItem = {
			...
			teacher_id: null, // Will be set in Pass 2
			start_time:
				convertToISO8601(fields["Start Date Time"]) || new Date().toISOString(),
			end_time:
				convertToISO8601(fields["End Date"]) || new Date().toISOString(),
...
async function updateForeignKeys(
	...
	for (const tableName of tables) {
		...
		while (hasMore) {
			...
			if (error) {
				console.error(
					`  ❌ Failed to fetch ${tableName} for lookup:`,
					error.message,
				);
				hasMore = false;
				break;
	...
	if (students.length > 0) {
		...
		for (const student of students) {
			...
			if (student._airtable_desired_starting_level_id) {
				student.desired_starting_language_level_id =
					matchAirtableLevelIdToSupabase(
						student._airtable_desired_starting_level_id,
					);
			}
		...
		while (hasMore) {
			...
			if (error) {
				console.error(
					"    ❌ Failed to fetch students for lookup:",
					error.message,
				);
				break;
			}
	...
	if (templateMessages.length > 0) {
		...
		for (const message of templateMessages) {
			if (message._airtable_sequence_id) {
				const sequenceId = lookupMaps.template_follow_up_sequences?.get(
					message._airtable_sequence_id,
				);
				if (sequenceId) {
				...
				console.warn(
					`    ⚠️ Message ${message.airtable_record_id} has no sequence reference`,
				);
			}
		}

		console.log(
			`    Found ${messagesToInsert.length}/${templateMessages.length} messages with valid sequence references`,
		);
		...
		if (messagesToInsert.length > 0) {
			...
			if (error) {
				console.error(
					"    ❌ Failed to insert template messages:",
					error.message,
				);
				importStats.errors.push({
					table: "template_follow_up_messages",
					error: error.message,
				});
			} else {
				console.log(
					`    ✅ Inserted ${messagesToInsert.length} template messages`,
				);
				importStats.tables.template_follow_up_messages = {
	...
	if (cohorts.length > 0) {
		...
		for (const cohort of cohorts) {
			...
			if (cohort._airtable_product_id) {
				cohort.product_id =
					lookupMaps.products?.get(cohort._airtable_product_id) || null;
			}
			...
			if (cohort._airtable_starting_level_id) {
				...
				cohort.starting_level_id = matchAirtableLevelIdToSupabase(
					cohort._airtable_starting_level_id,
				);
			}
			if (cohort._airtable_current_level_name) {
				// Current Level: Use display name to match with display_name in Supabase
				cohort.current_level_id = matchLevelNameToSupabase(
					cohort._airtable_current_level_name,
				);
			}
		...
		if (cohortsToInsert.length > 0) {
			console.log(
				`    Attempting to insert ${cohortsToInsert.length} cohorts...`,
			);
			...
			if (error) {
				...
				console.log(
					`    ✅ Actually inserted ${actualInserted}/${cohortsToInsert.length} cohorts`,
				);
				importStats.tables.cohorts = {
					attempted: cohortsToInsert.length,
		...
		while (hasMoreCohorts) {
			...
			if (error) {
				console.error(
					"    ❌ Failed to fetch cohorts for lookup:",
					error.message,
				);
				break;
			}
	...
	if (enrollments.length > 0) {
		...
		for (const enrollment of enrollments) {
			...
			if (enrollment._airtable_student_id) {
				enrollment.student_id = lookupMaps.students?.get(
					enrollment._airtable_student_id,
				);
				if (!enrollment.student_id) {
					skipReason =
						"Student not found" +
						lookupMaps.students?.get(enrollment._airtable_student_id) +
						" " +
						enrollment._airtable_student_id;
				}
			} else {
			...
			if (enrollment._airtable_cohort_id) {
				enrollment.cohort_id = lookupMaps.cohorts?.get(
					enrollment._airtable_cohort_id,
				);
				if (!enrollment.cohort_id) {
					skipReason = skipReason
						? "Student and Cohort not found"
						: "Cohort not found";
				}
			} else {
				skipReason = skipReason
					? "No student or cohort reference"
					: "No cohort reference";
			}
			...
			if (enrollment.student_id && enrollment.cohort_id) {
				...
				trackSkippedRecord(
					"enrollments",
					enrollment.airtable_record_id,
					skipReason || "Missing required references",
					{
		...
		if (enrollmentsToInsert.length > 0) {
			...
			if (error) {
				...
				console.log(
					`    ✅ Inserted ${enrollmentsToInsert.length} enrollments`,
				);
	...
	if (assessments.length > 0) {
		...
		for (const assessment of assessments) {
			if (assessment._airtable_student_id) {
				assessment.student_id = lookupMaps.students?.get(
					assessment._airtable_student_id,
				);
			}
			if (assessment._airtable_level_id) {
				// Level: Use Airtable record ID to match with airtable_record_id in Supabase
				assessment.level_id = matchAirtableLevelIdToSupabase(
					assessment._airtable_level_id,
				);
			}
			if (assessment._airtable_interview_held_by) {
				assessment.interview_held_by =
					lookupMaps.teachers?.get(assessment._airtable_interview_held_by) ||
					null;
			}
			if (assessment._airtable_level_checked_by) {
				assessment.level_checked_by =
					lookupMaps.teachers?.get(assessment._airtable_level_checked_by) ||
					null;
			}
		...
		if (assessmentsToInsert.length > 0) {
			...
			if (error) {
				...
				console.log(
					`    ✅ Inserted ${assessmentsToInsert.length} assessments`,
				);
				importStats.tables.student_assessments = {
					attempted: assessmentsToInsert.length,
					succeeded: assessmentsToInsert.length,
					failed: 0,
					skipped: 0,
					skippedReasons: {},
				};
	...
	if (automatedFollowUps.length > 0) {
		...
		for (const followUp of automatedFollowUps) {
			if (followUp._airtable_student_id) {
				followUp.student_id = lookupMaps.students?.get(
					followUp._airtable_student_id,
				);
			}
			if (followUp._airtable_sequence_id) {
				followUp.sequence_id = lookupMaps.template_follow_up_sequences?.get(
					followUp._airtable_sequence_id,
				);
			}
		...
		if (followUpsToInsert.length > 0) {
			...
			if (error) {
				console.error(
					"    ❌ Failed to insert automated follow ups:",
					error.message,
				);
				importStats.errors.push({
					table: "automated_follow_ups",
					error: error.message,
				});
			} else {
				console.log(
					`    ✅ Inserted ${followUpsToInsert.length} automated follow ups`,
				);
				importStats.tables.automated_follow_ups = {
					attempted: followUpsToInsert.length,
					succeeded: followUpsToInsert.length,
					failed: 0,
					skipped: 0,
					skippedReasons: {},
				};
	...
	if (touchpoints.length > 0) {
		...
		for (const touchpoint of touchpoints) {
			if (touchpoint._airtable_student_id) {
				touchpoint.student_id = lookupMaps.students?.get(
					touchpoint._airtable_student_id,
				);
			...
			if (touchpoint._airtable_automated_follow_up_id) {
				touchpoint.automated_follow_up_id =
					lookupMaps.automated_follow_ups?.get(
						touchpoint._airtable_automated_follow_up_id,
					) || null;
			}
		...
		console.log(
			`    Found ${touchpointsToInsert.length}/${touchpoints.length} touchpoints with valid student references`,
		);
		...
		if (touchpointsToInsert.length > 0) {
			...
			if (error) {
				...
				console.log(
					`    ✅ Inserted ${touchpointsToInsert.length} touchpoints`,
				);
				importStats.tables.touchpoints = {
					attempted: touchpointsToInsert.length,
					succeeded: touchpointsToInsert.length,
					failed: 0,
					skipped: 0,
					skippedReasons: {},
				};
	...
	if (weeklySessions.length > 0) {
		...
		console.log(
			`    Cohorts lookup map has ${lookupMaps.cohorts?.size || 0} entries`,
		);
		...
		if (sessionsToInsert.length > 0) {
			...
			if (error) {
				console.error(
					"    ❌ Failed to insert weekly sessions:",
					error.message,
				);
				importStats.errors.push({
				...
				console.log(
					`    ✅ Inserted ${sessionsToInsert.length} weekly sessions`,
				);
				importStats.tables.weekly_sessions = {
					attempted: sessionsToInsert.length,
					succeeded: sessionsToInsert.length,
					failed: 0,
					skipped: 0,
					skippedReasons: {},
				};
	...
	if (classes.length > 0) {
		...
		for (const classItem of classes) {
			if (classItem._airtable_cohort_id) {
				classItem.cohort_id = lookupMaps.cohorts?.get(
					classItem._airtable_cohort_id,
				);
			}
			if (classItem._airtable_teacher_id) {
				classItem.teacher_id =
					lookupMaps.teachers?.get(classItem._airtable_teacher_id) || null;
			}

			if (!classItem.cohort_id) {
				trackSkippedRecord(
					"classes",
					classItem.airtable_record_id,
					"No cohort_id found",
					{
		...
		if (classesToInsert.length > 0) {
			...
			if (error) {
				...
				importStats.tables.classes = {
					attempted: classesToInsert.length,
					succeeded: classesToInsert.length,
					failed: 0,
					skipped: 0,
					skippedReasons: {},
				};
