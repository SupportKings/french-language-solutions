...
console.log(`AIRTABLE_API_KEY: ${AIRTABLE_API_KEY ? `${AIRTABLE_API_KEY.substring(0, 20)}... (${AIRTABLE_API_KEY.length} chars)` : "NOT SET"}`);
...
async function fetchSampleRecords(tableName: string, limit = 3): Promise<any[]> {
...
function mapAirtableTypeToTypeScript(airtableType: string, options?: any): string {
	...
	switch (airtableType) {
		case "singleSelect":
			if (options?.choices) {
				const choices = options.choices.map((c: any) => `"${c.name}"`).join(" | ");
