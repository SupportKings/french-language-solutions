...
import { students, automatedFollowUps, touchpoints } from "./schema";
...
async function seedTouchpoints() {
	...
	const touchpointsData = Array.from({ length: 50 }, () => {
		...
		const automatedFollowUp = faker.datatype.boolean({ probability: 0.3 }) && existingAutomatedFollowUps.length > 0 
