...
async function seedAttendance() {
	...
	try {
		...
		if (existingStudents.length === 0 || existingCohorts.length === 0) {
			console.log("❌ No existing students or cohorts found. Please run the main seed first: bun run seed");
			return;
		}

		console.log(`Found ${existingStudents.length} students, ${existingCohorts.length} cohorts, ${existingEnrollments.length} enrollments, ${existingClasses.length} classes`);
		...
		for (const enrollment of existingEnrollments.slice(0, 30)) { // Use first 30 enrollments
			const cohortClasses = existingClasses.filter(c => c.cohortId === enrollment.cohortId).slice(0, 6); // Get first 6 classes for this cohort
			...
			for (const classItem of cohortClasses) {
				...
				attendanceData.push({
					...
					status,
					notes: status === "not_attended" ? faker.helpers.arrayElement([
						"Student was sick",
						"Family emergency",
						"Work conflict",
						"Travel",
					...
					markedBy: status !== "unset" && existingTeachers.length > 0 
		...
		for (let i = 0; i < 50; i++) {
			...
			const status = faker.helpers.arrayElement(["attended", "not_attended", "unset"]);
			...
			attendanceData.push({
				...
				notes: status === "not_attended" ? faker.helpers.arrayElement([
					...
					null
				]) : (status === "attended" && faker.datatype.boolean({ probability: 0.1 }) ? 
		...
		if (attendanceData.length > 0) {
			const insertedAttendanceRecords = await db.insert(attendanceRecords).values(attendanceData).returning();
			console.log(`✅ Inserted ${insertedAttendanceRecords.length} attendance records`);
			
			// Show some statistics
			const attendedCount = attendanceData.filter(a => a.status === "attended").length;
			const absentCount = attendanceData.filter(a => a.status === "not_attended").length;
			const unsetCount = attendanceData.filter(a => a.status === "unset").length;
			
			console.log(`📊 Statistics:`);
			console.log(`   - Attended: ${attendedCount} (${Math.round(attendedCount/attendanceData.length*100)}%)`);
			console.log(`   - Absent: ${absentCount} (${Math.round(absentCount/attendanceData.length*100)}%)`);
			console.log(`   - Not marked: ${unsetCount} (${Math.round(unsetCount/attendanceData.length*100)}%)`);
		} else {
