	...
	async getByTallySubmissionId(c: Context) {
		try {
			...
			if (!tallyFormSubmissionId) {
				return c.json({ success: false, error: "Tally form submission ID is required" }, 400);
			}
			
			const student = await this.service.findByTallySubmissionId(tallyFormSubmissionId);
			...
			return c.json({ success: false, error: "Failed to retrieve student" }, 500);
		}
	}
}